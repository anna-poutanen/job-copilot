...
from services import (storage, jobs, research, generate, email_service, llm,
                     calendar_service, email_inbox, briefing, todos)
...
def pipeline_get():
    return jsonify({"stages": storage.PIPELINE_STAGES, "items": storage.pipeline_jobs()})


@app.post("/api/pipeline/track")
def pipeline_track():
    """Promote a found job into the pipeline (first stage)."""
    d = request.json or {}
    storage.update_job(int(d["id"]), status="saved")
    return jsonify({"ok": True})
...
def pipeline_add():
    ...
    if not (d.get("company") or d.get("role")):
        return jsonify({"error": "Add at least a company or a role."}), 400
    jid = storage.add_application(d.get("company", ""), d.get("role", ""),
...
def jobs_update():
    ...
    if d.get("status") and d["status"] not in storage.PIPELINE_STAGES:
        return jsonify({"error": "Unknown stage."}), 400
    storage.update_job(int(d["id"]), status=d.get("status"), notes=d.get("notes"))
    return jsonify({"ok": True})
...
@app.get("/api/todos")
def todos_get():
    return jsonify({"todos": storage.list_todos()})


@app.post("/api/todos/extract")
def todos_extract():
    try:
        res = todos.extract_from_inbox()
        if res.get("error"):
...
def todos_add():
    ...
    added = storage.add_todo(task, due=d.get("due", ""), source="manual",
...
def todos_update():
    d = request.json or {}
    storage.set_todo_done(int(d["id"]), bool(d.get("done")))
    return jsonify({"ok": True})
...
# ---------- daily agenda / inbox / brief ----------
@app.get("/api/agenda")
def get_agenda():
    days = int(request.args.get("days", 1))
    return jsonify(calendar_service.agenda(days=days))


@app.get("/api/inbox")
def get_inbox():
    days = int(request.args.get("days", 2))
    return jsonify(email_inbox.digest(since_days=days))


@app.post("/api/briefing")
def post_briefing():
    try:
        ag = calendar_service.agenda(days=1)
        inbox = email_inbox.digest(since_days=2)
        return jsonify({"brief": briefing.morning_brief(ag, inbox),
                        "agenda": ag, "inbox": inbox})
    except llm.LLMNotConfigured as e:
        return jsonify({"error": str(e)}), 400
